...
COLUMNS = [
    'subject_id', 'subject_label', 'object_id', 'object_label', 'predicate_id', 'mapping_justification'
]


def write_sssom(df: pd.DataFrame, prefix_map: dict[str, str], path: Path) -> None:
...
def fix_cvx():
    ...
    pm = {"VO": "http://purl.obolibrary.org/obo/VO_", "cvx": "https://biopragmatics.github.io/providers/cvx/"}
    write_sssom(df, pm, HERE.joinpath("vo-cvx.sssom.tsv"))
...
def fix_fda():
    ...
    pm = {"VO": "http://purl.obolibrary.org/obo/VO_", "stn": "https://bioregistry.io/stn:"}
    write_sssom(df, pm, HERE.joinpath("vo-fda.sssom.tsv"))
...
def fix_omop():
    ...
    pm = {"VO": "http://purl.obolibrary.org/obo/VO_", "omop": "http://api.ohdsi.org/WebAPI/vocabulary/concept/"}
    write_sssom(df, pm, HERE.joinpath("vo-omop.sssom.tsv"))
...
def fix_rxnorm():
    ...
    write_sssom(df, pm, HERE.joinpath("vo-rxnorm.sssom.tsv"))
...
def fix_usda():
    ...
    usda_uri_prefix = "???:"
    df['object_id'] = df['USDA Code'].map(lambda s: usda_prefix + s.removeprefix("USDA: "))
    _add_sssom_boilerplate(df)
    pm = {
        "VO": "http://purl.obolibrary.org/obo/VO_",
        usda_prefix: usda_uri_prefix,
    }
    write_sssom(df, pm, HERE.joinpath("vo-usda.sssom.tsv"))
